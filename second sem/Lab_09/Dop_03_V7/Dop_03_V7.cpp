...
void readFromFile(Address** phead, Address** plast) {
    ...
    *phead = *plast = NULL;
    ...
    while (!feof(fp)) {
        ...
        if (1 != fread(t, sizeof(Address), 1, fp)) break;
...
Address* findMax(Address** phead, int fieldSelection) {
    if (*phead == NULL) {
        return NULL;
    }
    Address* maxElement = *phead;
    Address* current = (*phead)->next;
    while (current) {
        int cmp = 0;
        if (fieldSelection == 1) {      
            cmp = strcmp(current->name, maxElement->name);
        }
        else if (fieldSelection == 2) { 
            cmp = strcmp(current->city, maxElement->city);
        }
        if (cmp > 0) {
            maxElement = current;
        }
...
int main(void) {
    ...
    while (true) {
        ...
        case 7: {
            int field;
            cout << "Выберите поле для поиска максимального элемента:" << endl;
            cout << "1 - имя" << endl;
            cout << "2 - город" << endl;
            cin >> field;
            cin.ignore(cin.rdbuf()->in_avail());
            Address* maxElem = findMax(&head, field);
            if (maxElem) {
                cout << "Максимальный элемент:" << endl;
                cout << maxElem->name << " " << maxElem->city << endl;
            }
            else {
                cout << "Список пуст или ошибка поиска." << endl;
            }
            break;
        }
        case 8:
